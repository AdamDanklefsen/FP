...
void Switch::Draw(Graphics & gfx) {
	...
	case SwitchState::OFF:
		...
		break;
	default:
		throw std::invalid_argument("We should not be here");
	}
	if (type == SwitchType::Prop) {
...
bool Switch::Press() {
	...
	if (RL.TestCol(xy) && m.LeftIsPressed()) {
		toggle = SwitchState::ON; return true;
	} else if (RR.TestCol(xy) && m.LeftIsPressed()) {
		toggle = SwitchState::OFF; return true;
	}
}
